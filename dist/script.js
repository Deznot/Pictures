...
/* harmony import */ var _modules_drag__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! ./modules/drag */ "./src/js/modules/drag.js");
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_forms__WEBPACK_IMPORTED_MODULE_2__["default"])(resSum);
  ...
  Object(_modules_calc__WEBPACK_IMPORTED_MODULE_6__["default"])('#size', '#material', '#options', '.promocode', '.calc-price', resSum);
  ...
  Object(_modules_drag__WEBPACK_IMPORTED_MODULE_12__["default"])();
});
...
"use strict";
__webpack_require__.r(__webpack_exports__);
var calc = function calc(size, material, options, promocode, result, resSum) {
...
var drag = function drag() {
  ...
  var wrapDropText = debounce(dropText, 400);
  fileUpload.forEach(function (item) {
    item.addEventListener('mouseover', function () {
      return wrapDropText(item);
    });
    item.addEventListener('mouseout', function () {
      return delDropText(item);
    });
  });
  ...
  fileInputs.forEach(function (item) {
    item.addEventListener('drop', function (e) {
      ...
      var name = arr[0].slice(0, 6) + dots + arr[1];
...
var filter = function filter(menuSelector, contentSelector, noSelector) {
...
function forms(resSum) {
  var forms = document.querySelectorAll('form'),
      ...
      upload = document.querySelectorAll('input[name = upload]'); // checkNumInputs('input[name="user_phone"]');
  ...
  var clearInputs = function clearInputs() {
    ...
    for (var _i = 0, _Object$keys = Object.keys(resSum); _i < _Object$keys.length; _i++) {
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      if (item.files[0].name.split('.')[0].length > 6) {
        dots = '...';
      } else {
        dots = '.';
      }
...
var mask = function mask(selector) {
  ...
  inputs.forEach(function (input) {
    ...
    input.addEventListener('keyup', createMask, function (e) {
      if (e.code === 'ArrowLeft') {
        e.preventDefault();
      }
    });
...
var modals = function modals() {
  ...
  function fixAbsolute(absoluteSelector) {
    var absolute = document.querySelectorAll(absoluteSelector),
        scroll = calcScroll();
    absolute.forEach(function (item) {
      var rightS = getComputedStyle(item).right;

      if (document.body.style.overflow == 'hidden') {
        item.style.right = "calc(".concat(rightS, " + ").concat(scroll, "px)");
      } else {
        item.style.right = "calc(".concat(rightS, " - ").concat(scroll, "px)");
      }
    });
...
var pictureSize = function pictureSize(blockSelector) {
  var blocks = document.querySelectorAll(blockSelector);

  function showImg(block) {
    var img = block.querySelector('img'); // sizes-3.png"
...
var scrolling = function scrolling(elemSelector) {
  var upElem = document.querySelector(elemSelector);
  window.addEventListener('scroll', function () {
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  btn.addEventListener('click', function () {
    ...
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getResources"])('assets/db.json').then(function (res) {
